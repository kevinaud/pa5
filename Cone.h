...
public:
    ...
    Point textureAt(Point intersectWorld, Matrix TransformMatrix, TextureMap* textureMap) {
        ...
        if (intersectsCap(intersect)) {
            return textureOnCap(intersect, textureMap); 
        } else {
            return textureOnBody(intersect, textureMap); 
        }
    }

    bool intersectsCap(Point intersect) {
        if (intersect[1] + 0.5 < 0.0001 && intersect[1] + 0.5 > -0.0001) {
            return true; 
        }
        return false; 
    }

    Point textureOnCap(Point intersect, TextureMap* textureMap) {
        double x = intersect[0], y = intersect[1], z = intersect[2];
    ...
    Point textureOnBody(Point intersect, TextureMap* textureMap) {
        double x = intersect[0], y = intersect[1], z = intersect[2];

        double angle = getAngleOnCircle(x, z);
